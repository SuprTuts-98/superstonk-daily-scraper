...
def make_submission():
    ...
    image = "wordcloud.png"
    title = "A word cloud made from all of the comments from yesterdays daily thread"
...
while True:
    ...
    if day == 1:
        prev_month = month - 1
        prev_day = calendar.monthrange(year,prev_month)[1]
    if day == (calendar.monthrange(year,month)[1]):
        next_month = month + 1
        next_day = 1
    pause.until(datetime(year, next_month, next_day, 1, 55, 0))
    gen_cloud()
